...
function PaginatedList({
	...
	}, [fetchCall, filters, pagination.currentPage, pagination.pageSize, showInstance, setDirty]);

	return (
		<>
			{loading && <LoadingElement />}
			{data && data.length === 0 && !loading && <NoDataElement message={"No hikes found."} />}
			{error && !loading && (
				<ErrorElement message={"Something went wrong during the request. Try again later."} />
			)}
			{data && data.length > 0 && !loading && (
				<DataContainer>{data.map(DataElement)}</DataContainer>
			)}
			{data && data.length > 0 && !loading && (
				<Paginator
