...
async function endHike(req, res) {
	...
	try {
		...
		if (!hikesForUser.some(h => h._id.toString() === id)) {
			return res.status(StatusCodes.UNAUTHORIZED).json(new Error("Can't end another user's recorded hike"));
		}
		const registeredHike = await registeredHikeDAL.completeRegisteredHike(id);
		// Inform buddies
		await sendRegisteredHikeTerminatedEmail(await (await registeredHike.populate("hike")).populate("user"));
